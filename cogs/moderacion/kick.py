...
class Kick(commands.Cog):
    ...
    @app_commands.describe(miembro="La persona que quieres expulsar.", razon="El motivo de la expulsión.")
    @is_moderator()
    async def kick(self, interaction: discord.Interaction, miembro: discord.Member, razon: str = None):
        
        # --- (Secciones 1 a 3: Comprobaciones, DM y Expulsión - Sin cambios) ---
        ...
        if miembro == interaction.guild.owner:
            await interaction.response.send_message("No puedes expulsar al Dueño.", ephemeral=True)
            return
        if miembro.top_role >= interaction.guild.me.top_role:
            await interaction.response.send_message(f"No puedo expulsar a {miembro.mention}. Rol superior.", ephemeral=True)
            return

        if razon is None:
            razon = "Razón no especificada."
        razon_log = f"Expulsado por {interaction.user.name}. Razón: {razon}"

        try:
            embed_dm = discord.Embed(title="Has sido expulsado 👢", description=f"Has sido expulsado de **{interaction.guild.name}**.\nRazón: {razon}", color=discord.Color.red())
            await miembro.send(embed=embed_dm)
        except Exception:
            print(f"No se pudo enviar DM a {miembro.name}.")

        try:
            await miembro.kick(reason=razon_log)
        except Exception as e:
            await interaction.response.send_message(f"Ocurrió un error inesperado al expulsar: {e}", ephemeral=True)
            return

        # --- 4. Confirmación al Moderador (Mensaje efímero) ---
        embed_confirm = discord.Embed(
            title="✅ Acción Completada: Expulsión",
            description=f"**Miembro:** {miembro.mention} (`{miembro.id}`)\n**Razón:** {razon}",
            color=discord.Color.green()
        )
        embed_confirm.set_footer(text=f"Expulsado por {interaction.user.name}")
        ...
        if LOG_CHANNEL_ID:
            ...
            if log_channel:
                # Re-usamos el Embed de confirmación, pero esta vez lo enviamos al canal
                try:
    ...
    async def on_kick_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        if isinstance(error, app_commands.CheckFailure):
            ...
            await interaction.response.send_message("¡Error! Necesito 'Expulsar Miembros'.", ephemeral=True)
        elif isinstance(error, app_commands.MissingRequiredArgument):
            await interaction.response.send_message("Debes especificar a quién expulsar.", ephemeral=True)
        else:
            print(f"Error inesperado en /kick: {error}")
